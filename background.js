...
function debug(message, data = null) {
  if (CONFIG.debug) {
      ...
      console.log(`[${timestamp}] PhishGuard: ${message}`, data || '');
...
async function checkUrl(url) {
  ...
  try {
      ...
      if (data.query_status === "ok" && data.threat) {
          debug("URLhaus threat detected", data);
          return {
              isMalicious: true,
  ...
  // Secondary check with Google Safe Browsing (placeholder)
  try {
      if (url.includes('phishing-test.com')) {
          return {
              isMalicious: true,
              source: "Safe Browsing Check",
              details: {
      ...
      debug("Safe Browsing check failed", error);
  }
